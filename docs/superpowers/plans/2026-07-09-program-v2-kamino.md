...
- **⚠️ CORRECT active USDC reserve — `getReserveBySymbol('USDC')` RETURNS A DEAD RESERVE.** On the Kamino main market (`7u3He…5PfF`), `getReserveBySymbol('USDC')` returns `5xXxt9uV…` which is **DEPRECATED** (status=2, depositLimit=0, ~$0.10 available) — depositing into it fails `DepositLimitExceeded`. This was empirically caught by running a real deposit CPI on the fork. The **ACTIVE** USDC reserve is:
  - reserve `D6q6wuQSrifJKZYpR1M8R4YawnLDtDsMmWM1NbBmgJ59` (status=0, depositLimit 1e15, ~$13M available)
  - liquiditySupply `Bgq7trRgVMeq33yt235zM2onQ4bRDBsY5EWiTetF4qw6`
  - collateralMint (cUSDC) `B8V6WVjPxW1UGwVDfxH2d2r8SyT4cqn7dQRK6XneVa7D`
  - usdcMint `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
  - lendingMarketAuthority `9DrvZvyWh1HuAoZxvYWMvkf2XCzryCpGgHqrMjyDWpmo` (PDA `[b"lma", market]`)
  - Task 0 must enumerate `market.reserves` and pick the USDC reserve with `status == 0`, not call `getReserveBySymbol`.
- **refresh_reserve oracle accounts for USDC:** pyth / switchboardPrice / switchboardTwap are ALL None; the reserve prices via **scope** only (`3t4JZcueEzTbVP6kLxXrL3VpWx45jDer4eqysweBchNH`). **Pass the KLEND PROGRAM ID as the sentinel for None oracles** (Anchor optional-account convention — verified against the SDK's own `refreshReserve` builder AND by on-chain execution: passing the system program `1111…` fails `InvalidPythPriceAccount`). So refresh_reserve accounts = `[reserve(mut), lendingMarket, KLEND, KLEND, KLEND, scope]`.
...
- **✅ REAL KAMINO CPI PROVEN ON THE FORK (2026-07-09).** A dev-only `kamino_roundtrip` instruction (`programs/locked_in/src/kamino_probe.rs`, driven by `programs-tests/scripts/kamino-roundtrip.mjs`) was deployed to the fork and executed a real `deposit_reserve_liquidity` CPI into the active reserve: **refresh_reserve succeeded (scope priced USDC at 0.9998), the account order passed klend validation, the lock PDA signed, and klend ran `TransferChecked` (25 USDC → reserve) + `MintTo` (cUSDC → PDA).** This validates: CPI account order for both deposit and redeem, the None-oracle sentinel, `refresh_reserve` sequencing, and PDA `invoke_signed`. The tx then panics in klend's own `lending_checks.rs:462` (`vault_balance − internal_available_amount` subtract-overflow) — a **cloned-state limitation of forking a strict-accounting lending protocol** (the reserve account and its supply vault are snapshotted independently, so klend's exact-accounting invariant doesn't hold on the fork). It is NOT a defect in our CPI and would not occur on real mainnet. To get a fully-green fork roundtrip, Task 0 must either patch the reserve's internal `available_amount` to match the cloned vault via `surfnet_setAccount`, or run deposit/redeem tests through the devnet mock reserve (Task 12) instead. Real-Kamino settlement is validated end-to-end only on mainnet (staged beta, small caps).
- **Deploy note:** the repo's `target/deploy/locked_in-keypair.json` is `EUABEbHUjiUn9NijapRJT2MVqQ5nSdqH3gSzTxyGucsN`, which does NOT match `declare_id!` (`3RC9XkPZ…`). To deploy to a fork/devnet you must either set `declare_id` to the deploy keypair's pubkey and rebuild, or `anchor keys sync`. Mainnet gets a fresh keypair with matching `declare_id`.
